  ...
  return (
    ...
    >
      <div className="mx-auto flex h-24 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="relative h-[54px] w-[200px] sm:h-[72px] sm:w-[270px] overflow-hidden rounded-md bg-white shadow-sm ring-1 ring-blue/8 transition-shadow group-hover:shadow-md">
            <Image
              src="/images/logo.png"
        ...
        <div className="flex items-center gap-4 sm:gap-5">
          ...
          >
            <svg aria-hidden="true" className="h-5 w-5 text-blue" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
              {open ? (
                <>
                  <path d="M18 6L6 18" />
                  <path d="M6 6l12 12" />
      ...
      <div
        id="mobile-nav"
        className={`fixed inset-x-0 top-[96px] bottom-0 z-40 bg-cream/98 transition-opacity duration-300 lg:hidden ${
          open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
        style={{ backdropFilter: "blur(16px)" }}
      >
        <nav className="flex h-full flex-col p-6 sm:px-8" aria-label="Mobile navigation">
